...
/// No 3D engine involved: vertices are projected with a hand-rolled rotation, triangles are
/// depth-sorted (painter's algorithm — a convex cube never needs more), and each triangle is
/// drawn by affine-transforming the skin bitmap so the UV triangle lands exactly on the
...
public sealed class PlayerHeadView : Control
{
    ...
    public override void Render(DrawingContext context)
    {
        ...
        foreach (var t in order)
        {
            ...
            // Backface cull: skip triangles facing away from the viewer.
            var p0 = projected[i0];
            var p1 = projected[i1];
            var p2 = projected[i2];
